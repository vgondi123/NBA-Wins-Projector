...
The next part of the project involved exploratory data analysis in order to figure which variables would be used in the predictive model. I first wanted to observe scatter plots of every variable against the total wins variable(the variable that will be predicted). The age vs wins scatterplot is an example of two variables having a positive correlation, which is what want. The coach experience vs wins scatterplot is an example of two variables having no real correlation, which is what we don't want. I then calculated the correlational values of every variable. We can clearly see that both Coach Exp and Coach W/L% have a very weak correlation with total wins. This means that these features will not be used in the predictive mmodel. I also looked at histograms of the distributions of each variable to confirm which variables are not good to use. The coach exp histogram is highly skewed to the right, showing that it is not good to use as it is normally distributed.
...
The second type of model I used was k-Nearest Neighbors. The variables that produced the best output with this model are Avg WS and Last Year's W/L%. This model produced a mean square of about 41 and a mean squred error of about 6. This shows that k-Nearest model is better at predicting wins compared to the linear regression model.. I then looked at how the model predicted wins for each team in 2021-2022 season versus actual wins. The model also did a fairly good job of figuring out which teams are the best and which are the worst. 
